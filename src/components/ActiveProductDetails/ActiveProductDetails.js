...
export const ActiveProductDetails = ({
  editMode,
  setEditMode,
  setDetailsModal,
  user,
      ...
      <div className={css.container}>
        ...
        <div className={css.infoBlock}>
          ...
          {!editMode ? (
            <div className={css.paramsBlock}>
              <p>
                {currentLang === 'ru'
                  ? product.name?.RU || 'no_name'
                  : currentLang === 'uk'
                    ? product.name?.UA || 'no_name'
                    : product.name?.DE || 'no_name'}
              </p>
              <p className={css.price}>{product.price.UAH || '-'}грн.</p>
              {isLoggedIn && (
                <div className={css.paramsWrapper}>
                  <p>
                    {t('quantity in stock')}: {product.quantityInStock}
                  </p>
                  <p>{t('params')}:</p>
                  {product?.color && (
                    <p>
                  ...
                  <p>
                    {t('price in Motea')}: {product?.moteaPrice?.UAH}грн{' '}
                    {product?.availabilityInMotea
                      ? '(' + product?.availabilityInMotea + ')'
                      : '(' + t('unknown') + ')'}
                  </p>
                  <p>
                    {t('vendor price')}: {Math.round(product?.vendorprice)}грн
                  </p>
                  <p>
                    {t('keywords')}:{' '}
                    {currentLang === 'ru'
                      ? product.metaKeywords.RU
                      : product.metaKeywords.UA}
                  </p>
                  <p>{t('dimensions')}:</p>
                  <p>
                  ...
                  <p>{t('set')}:</p>
                  {product.isSet && product.isSet[0] !== null ? (
                    <ul>
            ...
            <div className={css.paramsBlock}>
              <label>
                ...
                <textarea
                  ...
                  onChange={e => {
                    ...
                    setUpdated(prev => ({
                      ...prev,
                      'name.RU': e.target.value,
                    }));
                  }}
                  value={
                    !product.name?.RU || product.name.RU === ''
                      ? product.name.translatedRU
                      : product.name.RU
                  }
                />
              ...
              <label>
                ...
                <textarea
                  ...
                  onChange={e => {
                    ...
                    setUpdated(prev => ({
                      ...prev,
                      'name.UA': e.target.value,
                    }));
                  }}
                  value={
                    !product.name?.UA || product.name.UA === ''
                      ? product.name.translatedUA
                      : product.name.UA
                  }
                />
              </label>
              <div className={css.paramsWrapper}>
                ...
                <label>
                  ...
                  <textarea
                    onChange={e => {
                      ...
                      setUpdated(prev => ({
                        ...prev,
                        'metaKeywords.RU': e.target.value,
                      }));
                    }}
                ...
                <label>
                  ...
                  <textarea
                    onChange={e => {
                      ...
                      setUpdated(prev => ({
                        ...prev,
                        'metaKeywords.UA': e.target.value,
                      }));
                    }}
                ...
                {product?.marketplaces &&
                  Object.entries(product.marketplaces).map(m => (
                    <div
                      key={m[0]}
                      className={css.horoshopWrapper}
                      onClick={() => toggleMarketplace(m[0])}
                    >
                      {m[0].toUpperCase()}
                      <div className={css.progress}>
                        <div className={css.progressLine}>
                          <div
                            className={`${css.progressPoint} ${m[1] && css.progressRightPoint}`}
                          ></div>
                        </div>
                      </div>
                    </div>
        ...
        {!editMode ? (
          <div>
            ...
            <div
              className={css.descriptionBody}
              dangerouslySetInnerHTML={{
                __html: description || 'no_description',
              }}
            />
          </div>
        ) : (
            ...
            <label>
              ...
              <textarea
                ...
                onChange={e => {
                  ...
                  setUpdated(prev => ({
                    ...prev,
                    'description.RU': e.target.value,
                  }));
                }}
                rows={8}
                value={
                  !product.description?.RU || product.description.RU === ''
                    ? product.description.translatedRU
                    : product.description.RU
                }
              />
              {(!product?.description?.RU || product.description.RU === '') &&
            ...
            <label>
              ...
              <textarea
                onChange={e => {
                  ...
                  setUpdated(prev => ({
                    ...prev,
                    'description.UA': e.target.value,
                  }));
                }}
                rows={8}
                value={product.description.UA}
              />
              {(!product?.description?.UA || product.description.UA === '') &&
                !isLoading && (
                  <button
                    className={css.translateBtn}
                    onClick={() =>
